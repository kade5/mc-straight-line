    ...
    override fun afterChangeWorld(player: ServerPlayerEntity, origin: ServerWorld, destination: ServerWorld) {
        if (destination?.registryKey == World.NETHER) {
            ...
            if (blockState != null && blockState.block == Blocks.NETHER_PORTAL) {
                StraightLineManager.INSTANCE.setNetherLine(intArrayOf(player.blockPos.x, player.blockPos.z))
                StraightLineChallenge.LOGGER.info("Nether Line position has been set to " +
                        "(${player.blockPos.x}, ${player.blockPos.z})")
